...
export default function TeacherLogin() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // Case-sensitive check for Teacher credentials
    if (username === "Alraziteacher" && password === "Alrazi@123") {
      setLoginError("");
      // Secret Key: Browser ko batayen ke teacher verify ho chuka hai (Jab tab tab band na ho)
      sessionStorage.setItem("isTeacherVerified", "true");
      navigate("/"); // Moves to the Teacher Dashboard
    } else {
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans antialiased flex flex-col items-center justify-center p-4">
      <form
        ...
        className="w-full max-w-sm bg-white rounded-3xl shadow-lg overflow-hidden border border-slate-100"
      >
        <div className="bg-[#1e3a8a] text-white py-10 px-6 text-center flex flex-col items-center relative overflow-hidden">
          ...
          <h2 className="text-lg font-bold uppercase tracking-wider z-10">
            Al Razi Academy
          </h2>
          <p className="text-xs italic text-slate-300/90 font-light mt-1 z-10">
        ...
        <div className="p-6 md:p-8 space-y-5">
          <div className="flex items-center space-x-2 text-slate-500 font-semibold tracking-wider text-xs uppercase mb-1">
            ...
            <span>Teacher Login Portal</span>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="block text-[11px] font-bold tracking-wide text-slate-500 uppercase">
              Username
            </label>
            <input
              type="text"
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center space-x-2 bg-[#22c55e] hover:bg-[#1ca84f] text-white py-3 rounded-xl font-bold tracking-wide text-sm transition-all duration-200 mt-2 shadow-sm active:scale-[0.99]"
          >
